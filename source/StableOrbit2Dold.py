...
a_i = 1000 #initial altitude
a_f = 300 #final altitude
...
r.set_as_design_variable(lower=R, scaler=1E-4)
dr = csdl.Variable(name='dr', value=np.zeros((num)))
dr.set_as_design_variable(scaler=1E-2)
...
m.set_as_design_variable(lower=0, scaler=1E-4)

# controls
Fr = csdl.Variable(name='Fr', value=np.zeros(num)+ 0.001)
Fr.set_as_design_variable(lower=-max_thrust, upper=max_thrust, scaler=1E-2)
Ftheta= csdl.Variable(name='Ftheta', value= np.zeros(num) + 0.001)
Ftheta.set_as_design_variable(lower=-max_thrust, upper=max_thrust, scaler=1E-2)
...
dr_0 = dr[0]   # in m
dr_0.set_as_constraint(equals=0, scaler=1E-3)
theta_0 = theta[0]
...
for i in range(num - 1):
    ...
    # a_r = ddr + r*(dtheta)^2
    val = Fr[i]/m_total - u/((r[i]*1000)**2) + (r[i]*1000)*(dtheta[i]**2)
    ddr = ddr.set(csdl.slice[i], val)

    ## acceleration (theta direction)
    # a_theta = r*(ddtheta) + 2*dr*dtheta`
    ddtheta = 1/(r[i]*1000) * (Ftheta[i]/m_total - 2*dr[i]*dtheta[i])
    ...
    r0 = r[i + 1] - r[i] - (dr[i]*(1E-3)) * dt # converting dr to km
    r0.set_as_constraint(equals=0, scaler=1E-2)
