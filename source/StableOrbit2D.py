...
# First segment: linearly increase speed (downward)
thrust_values[:first_seg] = np.linspace(-max_thrust, 0, first_seg)

# Middle segment: maintain constant speed
thrust_values[first_seg:second_seg] = 0

# Last segment: linearly decrease back to zero
thrust_values[second_seg:] = np.linspace(0, -max_thrust, num - second_seg)
...
# thrust_angle.set_as_design_variable(scaler=theta_scale)
...
# Ftheta = csdl.Variable(name='Ftheta', value= np.linspace(-max_thrust/2, 0, num=num))
...
thrust_mag_f.set_as_constraint(equals=0, scaler=F_scale)
...
for i in range(num - 1):
    ...
    # with radial thrust control
    # val = Fr / m_total - ((u * 10 ** 9) / ((r[i] * 1000) ** 2)) + (r[i] * 1000) * (dtheta[i] ** 2)

    # with no radial thrust control
