...
for i in csdl.frange(num - 1):
    m_total_1 = m[i] + m_empty
    m_total_2 = m[i+1] + m_empty
    ## acceleration (radial direction)
    # a_r = ddr + r*(dtheta)^2
    ddr_1 = - (u * 10 ** 9) / ((r[i] * 1000) ** 2) + (r[i] * 1000) * (dtheta[i] ** 2)
    ddr_2 = - (u * 10 ** 9) / ((r[i+1] * 1000) ** 2) + (r[i+1] * 1000) * (dtheta[i+1] ** 2)

    # no radial thrust control (no F component)
    # val = - ((u * 10 ** 9) / ((r[i] * 1000) ** 2)) + (r[i] * 1000) * (dtheta[i] ** 2)
    ddr = ddr.set(csdl.slice[i], ddr_1)
    ddr = ddr.set(csdl.slice[i+1], ddr_2)

    ## acceleration (theta direction)
    # a_theta = r*(ddtheta) + 2*dr*dtheta
    ddtheta_1 = 1 / (r[i] * 1000) * ((F_theta[i] * 1000) / m_total_1 - 2 * dr[i] * dtheta[i])
    ddtheta_2 = 1 / (r[i+1] * 1000) * ((F_theta[i+1] * 1000) / m_total_2 - 2 * dr[i+1] * dtheta[i+1])
    # ddtheta = 1 / (r[i] * 1000) * (thrust_mag[i] / m_total - 2 * dr[i] * dtheta[i])

    # mass consumption
    Ftotal_1 = csdl.sqrt(F_theta[i] ** 2)
    dm_1 = -(Ftotal_1 * 1000) / (Isp * g0)
    Ftotal_2 = csdl.sqrt(F_theta[i+1] ** 2)
    dm_2 = -(Ftotal_2 * 1000) / (Isp * g0)

    # create the residuals for the dynamic constraints:
    r_res = r_res.set(csdl.slice[i], r[i + 1] - r[i] - 0.5*(dr[i] + dr[i+1]) * 1e-3 * dt)
    dr_res = dr_res.set(csdl.slice[i], dr[i + 1] - dr[i] - 0.5*(ddr_1 + ddr_2) * dt)
    theta_res = theta_res.set(csdl.slice[i], theta[i + 1] - theta[i] - 0.5*(dtheta[i]+dtheta[i+1]) * dt)
    dtheta_res = dtheta_res.set(csdl.slice[i], dtheta[i + 1] - dtheta[i] - 0.5*(ddtheta_1 + ddtheta_2) * dt)
    m_res = m_res.set(csdl.slice[i], m[i + 1] - m[i] - 0.5*(dm_1 + dm_2) * dt)
